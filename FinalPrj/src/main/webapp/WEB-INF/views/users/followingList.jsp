...
<script>
	...
	$('body').on('click', '#follow-btn', function() {
		var friend = $(this).data("follow");
		console.log(friend);
		follow(true, friend);
	});

	$('body').on('click', '#unfollow-btn', function() {
		var friend = $(this).data("unfollow");
		console.log(friend);
		follow(false, friend);
	});
		...
		if (check) {
			$.ajax({
				...
				success : function(result) {
					...
					if (result === "FollowOK") {
						$(".follow-area").remove("#follow-btn[data-follow='" + friend + "']");
						$(".follow-area").html('<a class="button" id="unfollow-btn" data-unfollow="'+ friend +'">Unfollow</a>');
					}
				}
			}); // end of follow ajax
		} else {
			$.ajax({
				...
				success : function(result) {
					...
					if (result === "UnFollowOK") {
						$(".follow-area").remove("#unfollow-btn[data-follow='" + friend + "']");
						$(".follow-area")
								.html(
										'<a class="button" id="follow-btn" data-follow="'+ friend +'">Follow</a>');
					}
				}
								...
								<div class="card-row">
									...
									<c:forEach items="${followingList }" var="friend" varStatus="status">
										<div class="card-flex friend-card">
											...
											<div class="friend-stats follow-area">
			                                	<c:choose>
				                                	<c:when test="${friend.followCheck > 0}">
					                               		<a class="button" id="unfollow-btn" data-unfollow="${friend.user_id }">Unfollow</a>
				                               		</c:when>
				                                	<c:when test="${friend.user_id eq user.user_id}">
				                               		</c:when>
			                                		<c:otherwise>
					                               		<a class="button" id="follow-btn" data-follow="${friend.user_id }">Follow</a>
			                                		</c:otherwise>
			                                	</c:choose>
											</div>
										</div>
