					...
					function() {
						...
						$('#friendSearch')
								.on(
										...
										function() {
											var gender = $(
													'input[name=genderval]:checked')
													.val();
											var dgender = $(
													'input[name=dgenderval]:checked')
													.val();
											var country_op = $("#append-op")
													.children();
											var discountry_op = $("#append-dop")
													.children();
											var country = "";
											var discountry = "";
											var lan = $(
													"#language1-val option:selected")
													.val();
											var topic = "";
											var distopic = "";

											$("input[name=topic-label]:checked")
													.each(
															function() {
																topic += $(this)
																		.val()
											...
											$(
													"input[name=dtopic-label]:checked")
													.each(
															function() {
																distopic += $(
																		this)
											...
											country_op.each(function(i, v) {
												country += v.id + ",";
											});
											discountry_op.each(function(i, v) {
												discountry += v.id.substr(3)
														+ ",";
											});
																				...
																				function(
																					...
																					if (locale == 'kr') {
																						$(
																								'#country-op')
																								.append(
																										'<option value="'+value.CountryNameEN+'" value2="'+key+'">'
																												+ value.CountryNameKR
																												+ '</option>');
																						$(
																								'#dcountry-op')
						...
						$('#country-op')
								.on(
										...
										function() {
											...
											var val2 = $(
													"#country-op > option:selected")
													.attr("value2"); //삭제구분value
						...
						$('#searchRow #searchItem').slice(startFilter,
								endFilter).addClass('shown');
						$('.shownLength').text(endFilter);
						$('#searchRow #searchItem').not('.shown').hide();

						$('#buttonToogle .load-more-button')
								.on(
										'click',
										function() {
											if (elementLength > endFilter) {
												...
												$('.shownLength')
														.text(
																(endFilter > elementLength) ? elementLength
																		: endFilter);
												if (elementLength <= endFilter) {
													$(this).remove();
												}
						...
						$('#allTab')
								.on(
										...
										function() {
											...
											$('#allRow #allItem').slice(
													startFilter, endFilter)
													.addClass('shown');
											$('.shownLength2').text(endFilter);
											$('#allRow #allItem').not('.shown')
													.hide();

											$(
													'#buttonToogle1 .load-more-button')
													.on(
															'click',
															function() {
																if (elementLength > endFilter) {
						...
						$('#newTab')
								.on(
										...
										function() {
											...
											$('#newRow #newItem').slice(
													startFilter, endFilter)
													.addClass('shown');
											$('.shownLength3').text(endFilter);
											$('#newRow #newItem').not('.shown')
													.hide();

											$(
													'#buttonToogle2 .load-more-button')
													.on(
															'click',
															function() {
																if (elementLength > endFilter) {
...
<body>
	...
	<div class="view-wrapper"
		style="font-family: 'ONE-Mobile-Regular' !important;">
				...
				<div class="nav-inner is-friends">
					...
					<div class="option-tabs is-friends">
						<a class="option-tab is-active" id="searchTab"
							data-tab="all-friends"> <span><spring:message
									code="friends.search" /> </span>
						</a> <a class="option-tab" id="allTab" data-tab="starred-friends">
							<span><spring:message code="friends.all" /></span>
						</a> <a class="option-tab" id="newTab" data-tab="new-friends"> <span><spring:message
									code="friends.new" /></span>
						</a>
						<div class="option-naver"></div>
					</div>
					<div class="end-group">
						<div class="nav-item is-friend-count">${fn:length(allList) }<spring:message
								code="friends.people.count" />
						</div>
		...
		<form id="frm" name="frm" method="post" action="searchList.do">
			...
			<div class="filters-panel" style="overflow: scroll;">
				<div class="panel-inner">
					<div class="title">
						<span class="search-title"><spring:message
								code="friends.search.title" /></span> <a
							class="button is-rounded is-solid accent-button"
							id="friendSearch"><spring:message code="friends.search" /></a>
					</div>
					<div class="field" style="font-size: .7rem;">
						<div class="control">
							<label class="search-label"><spring:message
									code="friends.search.age" /></label> <input type="text" class="input"
								id="s_age" name="s_age" maxlength="2"
								onKeyup="this.value=this.value.replace(/[^0-9]/g,'');"><small>
								<spring:message code="friends.search.age.and" />
							</small> <input type="text" class="input" id="e_age" name="e_age"
								maxlength="2"
					...
					<div>
						<label class="search-label"><spring:message
								code="friends.search.gender" /></label> <label class="radio-label"><input
							class="radio" type="radio" name="genderval" value=""
							checked="checked">
						<spring:message code="friends.search.gender.all" /></label> <label
							class="radio-label"><input class="radio" type="radio"
							name="genderval" value="M">
						<spring:message code="friends.search.gender.m" /></label> <label
							class="radio-label"><input class="radio" type="radio"
							name="genderval" value="W">
						<spring:message code="friends.search.gender.w" /></label> <label
							class="radio-label"><input class="radio" type="radio"
							name="genderval" value="O">
						<spring:message code="friends.search.gender.o" /></label>
					</div>
					<div class="div-margin">
						<div class="control">
							<label class="search-label"><spring:message
									code="friends.search.country" /></label>
							<div class="select">
								<select id="country-op">
									<option value=""><spring:message
											code="friends.search.all" /></option>
								</select>
							</div>
					...
					<div>
						<div class="div-margin">
							<div class="control">
								<label class="search-label"><spring:message
										code="friends.search.language" /></label>
								<div class="select">
									<select id="language1-val">
										<option value=""><spring:message
												code="friends.search.all" /></option>
										<option value="kr"><spring:message
												code="friends.search.language.ko" /></option>
										<option value="en"><spring:message
												code="friends.search.language.en" /></option>
									</select>
								</div>
							</div>
						</div>
					...
					<div>
						<label class="search-label"><spring:message
								code="friends.search.topic" /></label>
					...
					<div class="title">
						<hr>
						<span class="search-title"><spring:message
								code="friends.search.title2" /></span>
					</div>
					<!-- 제외조건 시작 -->
					<div class="field" style="font-size: .7rem;">
						<div class="control">
							<label class="search-label"><spring:message
									code="friends.search.age" /></label> <input type="text" class="input"
								id="s_dage" name="s_dage" maxlength="2"
								onKeyup="this.value=this.value.replace(/[^0-9]/g,'');"><small>
								<spring:message code="friends.search.age.and" />
							</small> <input type="text" class="input" id="e_dage" name="e_dage"
					...
					<div>
						<label class="search-label"><spring:message
								code="friends.search.gender" /></label> <label class="radio-label"><input
							class="radio" type="radio" name="dgenderval" value="M">
						<spring:message code="friends.search.gender.m" /></label> <label
							class="radio-label"><input class="radio" type="radio"
							name="dgenderval" value="W">
						<spring:message code="friends.search.gender.w" /></label> <label
							class="radio-label"><input class="radio" type="radio"
							name="dgenderval" value="O">
						<spring:message code="friends.search.gender.o" /></label>
					</div>
					<div class="div-margin">
						<div class="control">
		...
		<div id="friends-page" class="friends-wrapper main-container">
			...
			<div id="all-friends" class="card-row-wrap is-active">
				<div class="card-row-placeholder is-hidden">
					<spring:message code="friends.search.no" />
				</div>
										...
										<div class="img-container">
											<c:if test="${vo.uuid ne '-'}">
												<img class="avatar"
													...
													alt="">
												<img class="flag" src="${vo.flag }" alt="">
											</c:if>
										...
										<div class="friend-info">
											...
											<div class="friend-location">
												...
												<c:if test="${vo.topicCnt ne 0 }">
													<div class="divCnt">
														<h4>
															<spring:message code="friends.search.topic.count" />
															<span class="spanCnt">${vo.topicCnt }</span>
											...
											<div class="friend-location">
												<span><svg viewBox="0 0 24 24" width="15" height="15"
														...
														class="css-i6dzq1">
															...
															<circle cx="12" cy="10" r="3"></circle></svg></span> <span
													id="friend-city textFilter-match">${vo.city},
													${vo.country}</span> <span class="time"><p>
														<fmt:formatDate value="${vo.reg_date }"
															pattern="yyyy-MM-dd HH:mm:ss" var="reg_dt" />
														<script type="text/javascript">
															document
																	.write(timeForToday('${reg_dt }'));
														</script>
														<spring:message code="friends.search.new" />
													</p></span>
											</div>
										</div>
									</a>
